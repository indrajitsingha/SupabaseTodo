...
export const Addbrand = () => {
    ...
    const uploadImage = async (file) => {
        ...
        let { data, error } = await supabase.storage.from("brand").upload(filePath, file);
        console.log(data);
        if (error) {
    ...
    const insertTodos = async () => {
        ...
        const { data, error } = await supabase.from("Brands").insert({ Brand: input, BrandLogo: "https://cwcxoefreqluglpiruut.supabase.co/storage/v1/object/public/" + imageUrl });
    ...
    const updateTodos = async () => {
        ...
        const { data, error } = await supabase.from("Brands").update({ Brand: input, BrandLogo: "https://cwcxoefreqluglpiruut.supabase.co/storage/v1/object/public/" + imageUrl })
            .eq("id", edit.id);
        console.log(data, error);
        await getTodos();
        setInput("");
                ...
                <tbody>
                    {todos.map((todo) => (
                        <tr key={todo.id}>
                            <td>{todo.id}</td>
                            <td>{todo.Brand}</td>
                            <td><img src={todo.BrandLogo} alt="avatar" style={{ width: '50px' }} /></td>
                            <td>
